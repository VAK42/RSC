...
export default function Admin_Membership() {
  ...
  const [addingNewMembership, setAddingNewMembership] = useState<boolean>(false);
  useEffect(() => {
    fetchData();
  }, []);
  const fetchData = () => {
    axios
      .get<Membership[]>("http://localhost/rsc/mbs2.php")
      .then((response) => {
  ...
  const handleSaveAllChanges = () => {
    axios
      .post("http://localhost/rsc/mbs4.php", membership)
      .then(() => {
